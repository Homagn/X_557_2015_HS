...
{                                                               
    ...
    vec3 out_diffuse_color = diffuse_color  * diffuse_coefficient * diffuse_intensity2;                        
                                                                                                              
    // Ambient color                                                                                         
    vec3 out_ambient_color = vec3(ambient_color) * ambient_intensity2;                                        
    ...
    vec3 out_specular_color = specular_color * specular_coefficient * specular_intensity2;                    
  
	
      //No attenuation for point light so attenuation=1
	
	// Calculate the linear color
	vec3 linearColor = out_ambient_color  + 1 * ( out_diffuse_color + out_specular_color);  
...
{       
	...
	if(light_to_surface_angle > cone_angle){
                 float x= (light_to_surface_angle-cone_angle)/10;
                if(x < 1.0){ 
                 attenuation= 1-(3*x*x - 2*x*x*x);
                    }
                else{
	...
	vec3 anotherlight=light1();
	// Pass the color 
	pass_Color =  finalColor+anotherlight;
