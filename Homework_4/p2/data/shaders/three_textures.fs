...
void main(void)                                                   
{
    ...
    vec4 tex_color_b =  texture(texture_background, pass_TexCoord);
    vec4 tex_color_m =  texture(texture_midground, pass_TexCoord);
    vec4 tex_color_f =  texture(texture_foreground, pass_TexCoord);
    ...
    if(texture_blend == 0)
    {
        color = 0.1 * pass_Color + tex_color_b + tex_color_m +tex_color_f ;
    }
    else if(texture_blend == 1)
    {
        color = tex_color_b * tex_color_m * tex_color_f;
    }
    else if(texture_blend == 2)
    {
        color = ((tex_color_b.r)*tex_color_m +(tex_color_m.r)*tex_color_f +(tex_color_f.r)*tex_color_b
		(tex_color_m.r)*tex_color_b +(tex_color_f.r)*tex_color_m +(tex_color_b.r)*tex_color_f)/12;
    }
    else
    {
        color = 0.1 * pass_Color + tex_color_b;
    }
