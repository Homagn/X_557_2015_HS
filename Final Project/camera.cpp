...
#include <algorithm>
extern int _camheight;
extern int _camera_d;
extern int _camera_roll;
...
void CameraManipulator::init_camera_param(void)
{
    ...
    _traveling_speed = 100.0f;
    _camera_distance = 10.0f;
...
glm::mat4  CameraManipulator::getRotationMatrix(void)
{
	...
	glm::mat4 cama;
	//cama = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -25.0, 10.0));
	cama = tranform3;
	
	cama = cama*glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 25.0, -10.0));
	return cama;
	cama = cama*glm::rotate(glm::degrees(_currentAngle) * _rolling_speed, glm::vec3(0.0f, 1.0f, 0.0f));
	cama = cama*glm::translate(glm::mat4(1.0f), glm::vec3(ch, cd, cr));
	return glm::rotate(glm::degrees(_currentAngle) * _rolling_speed, glm::vec3(0.0f,1.0f,0.0f));
	//return cama;
	//return glm::rotate(glm::degrees(_currentAngle) * _rolling_speed, _cameraRotationAxis);
}
...
{
    //return glm::vec3(_camera_roll,_camheight,_camera_d);
	return glm::vec3(ch, cd, cr);
