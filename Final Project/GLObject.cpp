 ...
 */
void SetTrackballLocation(glm::mat4 orientation, glm::vec3 translation, glm::vec3 look)
{
	float jj = orientation[0][1];
	float kk = orientation[0][2];
	float ll = orientation[0][3];
    g_viewMatrix = glm::lookAt(translation, translation+look, glm::vec3(0.0f, 1.0f, 0.0f));
	...
	wl.y = -1.0;
	wl.z = 0.0;
	glm::mat4 viewM = glm::lookAt(wt, wt+wl, glm::vec3(0.0f, 1.0f, 0.0f));
    //g_rotated_view = g_viewMatrix * g_trackball;
	if (cammode < 0)
	{
