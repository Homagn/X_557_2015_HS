...
#ifdef WIN32
string  GLTexture::_glsl_names[4] = { "tex","tex1", "tex2" , "texture_blend"};

string  GLMultiTexture::_glsl_names[4] = { "texture_background","texture_midground", "texture_foreground", "texture_blend"};
#endif
...
int GLTexture::loadAndCreateTexture(string path_and_file)
{
	...
	texturenumber++;
	_texture.push_back(texturenumber);
    ...
    glGenTextures(1, &_texture[texturenumber] );
    
    // Set a texture as active texture.
    glBindTexture( GL_TEXTURE_2D, _texture[texturenumber] );
...
bool GLTexture::addVariablesToProgram(GLuint program, int variable_index)
{
    ...
    _textureIdx = glGetUniformLocation(program, _glsl_names[texturenumber].c_str() );
    checkUniform(_textureIdx, _glsl_names[texturenumber]);
     ...
     */
    glUniform1i(_textureIdx, texturenumber);
...
/*!
 ...
 @return int - the texture id when the texture was sucessfully loaded.
 */
void GLMultiTexture::changemode(int mode)
{
	_modelno = mode;
}
int GLMultiTexture::loadAndCreateTextures(string path_and_file_texture_1, string path_and_file_texture_2, string path_and_file_texture_3, int k)
{
	cout << "started loading textures";
...
bool GLMultiTexture::addVariablesToProgram(GLuint program, int variable_index)
{
	...
	cout << "texture 1 added";
    
    
    /*
     glActiveTexture tells OpenGL which texture unit we want to use. GL_TEXTURE1 is the second texture unit, so we will just use that.
     */
    glActiveTexture(GL_TEXTURE1);
	...
	cout << "texture 2 added";

	/*
	glActiveTexture tells OpenGL which texture unit we want to use. GL_TEXTURE2 is the third texture unit, so we will just use that.
	*/
	glActiveTexture(GL_TEXTURE2);
